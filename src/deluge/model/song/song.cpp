	...
	for (Output* output = firstOutput; output; output = output->next) {
		...
		if (output->shouldRenderInSong()) {
			...
			const uint8_t routing =
			    (routingClip && cvOutputsAvailable()) ? routingClip->cvRouting : ClipRoute::DEFAULT;
			...
			const uint32_t numMono = (uint32_t)numSamples * 2;
			...
			int32_t* const mixRaw = (int32_t*)outputBuffer;
			if (capture) {
			...
			if (capture) {
				...
				if (toCV) {
					cvStreamCapture((uint32_t)routingClip, mixRaw, scratch, (uint32_t)numSamples, sendGain,
