...
bool GlobalEffectable::modEncoderButtonAction(uint8_t whichModEncoder, bool on,
	...
	if (modKnobMode == 6 && whichModEncoder == 0 && currentSong != nullptr
	    && this == &currentSong->globalEffectable) {
		if (on && cvOutputsAvailable() && !cvGetStereoSplit()) {
			cvMasterKnobSocket ^= 1;
			display->displayPopup(l10n::get(cvMasterKnobSocket == 0 ? STRING_FOR_OUTPUT_LEVEL_CV1
