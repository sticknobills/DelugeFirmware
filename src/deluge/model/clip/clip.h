...
public:
	...
	/// Pre-release songs stored USB routing on the Clip. Read from file only, folded into the Clip's Output once
	/// outputs are linked, and never written back. kNoLegacyUsbRouting rather than 0 for "absent", because 0 is a
	/// legal saved value - a track taken out of the main mix and sent nowhere. Delete with the legacy read in
	/// clip.cpp at public release.
	static constexpr uint16_t kNoLegacyUsbRouting = 0xFFFF;
	uint16_t legacyUsbRouting;
