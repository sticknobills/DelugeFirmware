...
/// Which USB audio channels a Clip's track is copied out to, plus whether it stays in the machine's own main mix.
///
/// One bitmask rather than a channel number: a clip may feed several destinations, and several clips may feed one
...
public:
	...
	/// The single question "does this clip's audio still reach the Deluge's own outputs". One function so the merge
	/// with the AUX sends line has one place to collapse two routing masks into one.
	...
	/// UsbRoute bits. Read once per render window by the output loop, off whichever clip is playing on the track.
	uint16_t usbRouting;
